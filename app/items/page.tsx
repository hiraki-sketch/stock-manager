...
export default async function ItemsPage(props: unknown) {
  ...
  let query = supabase.from("items").select("*").eq("deleted", false);
  ...
  const name =
    typeof searchParams.name === "string"
      ...
      : undefined;
  const checker =
    typeof searchParams.checker === "string"
      ? searchParams.checker.trim()
      : undefined;
  const date =
  ...
  const { data: items, error: fetchError } = await query.order("updated_at", {
    ascending: false,
  });

  if (fetchError) {
    return (
      <p className="p-6 text-red-600 bg-red-50 border border-red-200 rounded">
        データ取得エラー: {fetchError.message}
      </p>
    );
  }
